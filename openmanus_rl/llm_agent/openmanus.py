    ...
    def _run_single_rollout(self, initial_prompt_ids: torch.Tensor, task_idx: int, client: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            for t in range(self.config.max_turns):
                ...
                action_text = action_contents[0] # This might be a single JSON string or a JSON array string

                # ---- NEW LOGIC FOR NO ACTION ----
                # Check if action_text is None, an empty string, or represents an empty JSON array "[]"
                if not action_text or (isinstance(action_text, str) and action_text.strip() == "[]"):
                    print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] No valid action generated by LLM. Terminating episode.")
                    ...
                    break # Exit the turn loop (the "for t in range(self.config.max_turns):" loop)
                # ---- END OF NEW LOGIC ----

                # Execute environment step using the provided client
                if action_text is None: action_text = "" # This line can remain, handles if a tool expects empty string for "do nothing"
                
                step_output = client.step(action_text) # This call is now guarded by the check above
    ...
    def postprocess_predictions(self, predictions: List[Any]) -> Tuple[List[str], List[str]]:
        ...
        for prediction in predictions:
            if isinstance(prediction, str):
                tool_call_content_pattern = r'<tool_call>(.*?)</tool_call>'
                matches = re.finditer(tool_call_content_pattern, prediction, re.DOTALL)
                extracted_tool_calls = [match.group(1).strip() for match in matches]

                if not extracted_tool_calls:
                    actions.append(None)
                    contents.append('')
                elif len(extracted_tool_calls) == 1:
                    actions.append('tool_call')
                    contents.append(extracted_tool_calls[0])
                else: # Multiple tool calls
                    actions.append('tool_call')
                    # Construct a JSON array string.
                    # Assumes each item in extracted_tool_calls is already a valid JSON string.
                    json_array_string = "[" + ", ".join(extracted_tool_calls) + "]"
                    contents.append(json_array_string)
            else:
