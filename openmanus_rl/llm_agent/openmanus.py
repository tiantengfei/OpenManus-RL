...
import os
from typing import List, Dict, Any, Tuple, Optional # Optional is already here, good.
...
class OpenManusAgent:
    ...
    def _run_single_rollout(self, initial_prompt_ids: torch.Tensor, task_idx: int, client: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            for t in range(self.config.max_turns):
                ...
                content_part, action_list = self.postprocess_predictions(response_text) # action_list is List[str]
                actions_to_execute = action_list # Pass the list of actions

                # You might log content_part if useful:
                # print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] Content part: {content_part}")

                # Check if any actions were generated
                if not actions_to_execute: # Check if the list is empty
                    print(f"[Agent._run_single_rollout][{task_idx}][Turn {t+1}] No valid actions generated by LLM. Terminating episode.")
                    # Optionally add a system message to trajectory to indicate this
                    trajectory.append({
                        "from": "system", 
                        "value": "Agent did not produce a valid action. Episode terminated."
                        # You could also add 'reward': 0.0 or some other specific value here if needed
                    })
                    done = True
                    # final_reward, final_env_score could be set here if specific values apply for no-action termination
                    break 
                
                # Execute environment step using the provided client, passing the list of actions
                # Assuming client.step() can now handle List[str] or a string representation of it.
                # If client.step() strictly expects a string, actions_to_execute might need to be json.dumps-ed.
                # Based on the prompt, we pass the list directly.
                step_output = client.step(actions_to_execute) 
    ...
    def postprocess_predictions(self, prediction: str) -> Tuple[str, List[str]]:
        """
        Process a single prediction string to separate natural language from all tool calls.
        ...
        Returns:
            A tuple containing:
                - content (str): Text before the *first* <tool_call> tag, stripped.
                                 If no <tool_call> tag, this is the full prediction string (stripped).
                                 An empty input string or non-string results in empty content.
                - actions_list (List[str]): A list of *all* raw string contents extracted from 
                                            *all* <tool_call>...</tool_call> blocks, each stripped.
                                            Empty if no tool calls are found.
        """
        content = ""
        actions_list = []
        ...
        if first_tag_idx != -1:
            ...
            # Use re.finditer to find all tool call contents
            # The pattern r'<tool_call>(.*?)</tool_call>' is correct for extracting content between tags.
            # re.DOTALL ensures that '.' matches newlines within the tool call content.
